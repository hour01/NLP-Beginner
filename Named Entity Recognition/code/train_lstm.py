...
# Model configs
parser.add_argument('--emb_dim', type=int, default=300, help='Tokens will be embedded to a vector.')
parser.add_argument('--hidden_dim', type=int, default=150, help="hidden size of lstm")
...
opt.tag2id = {'O': 0, 'B-PER': 1, 'I-PER': 2, 'B-ORG': 3, 'I-ORG': 4, 'B-LOC': 5, 'I-LOC': 6, 'B-MISC': 7, 'I-MISC': 8}
...
def train(config, model, train_set, val_set):
    ...
    val_dataloader = DataLoader(val_set, batch_size=config.batch_size, collate_fn=val_set.collate_fn)
...
def evaluate(config, model, val_dataloader):
    ...
    for batch in val_dataloader:
        ...
        l, pred = model(input_ids, labels_flatten, lengths, input_mask)
        loss += l
...
def test(config, model, test_features):
    test_dataloader = DataLoader(test_features, batch_size=config.batch_size, collate_fn=test_features.collate_fn)
    metric = Metric(config.id2word, config.id2tag)
    model.eval()
    loss = 0
...
def main():
    ...
    model = models.BiLSTM(opt).to(opt.device)
