...
def get_dict_wordvec():
    ...
    if os.path.exists(wordvec_path):
        ...
        word_vec = get_Glove(dict)
    return dict,revdict,word_vec

def get_pretrain_wordvec(dict):
...
def get_Glove(dict):
    ...
    word_vec = torch.randn([len(dict), 100])# 没在glove字典中的随机初始化
    for word,id in dict.items():
...
if __name__ == '__main__':
    if opt.mode == 'test':
        ...
        model = models.MY_CNN(word_vec,opt.n_filters,opt.filter_sizes,freeze_emb=opt.freeze_emb,drop_out=opt.dropout).to(device)
        if opt.restore == '':
