...
parser.add_argument('-bert_out', type=int, default=768, help="hidden of elmo output")
...
def collate_fn(batch_data):
    ...
    padded_sequence = tokenizer(sentences, padding=True, return_tensors="pt") # padding, token to idx
    ctgy = [i for i in Y]
    return padded_sequence['input_ids'], torch.as_tensor(ctgy, dtype=torch.float32)
...
def train(model_bert, model, iterator, optimizer, criterion):
...
def evaluate(model_bert, model, iterator, criterion):
...
def test_glue(model_bert, model, iterator):
...
if __name__ == '__main__':
    if opt.mode == 'test':
        ...
        model_bert = BertModel.from_pretrained(pretrain_path).to(device)
        if opt.model == 'cnn':
            model = models.MY_CNN(opt.bert_out,opt.n_filters,opt.filter_sizes,drop_out=opt.dropout).to(device)
        else:
            model = models.MY_LSTM(opt.bert_out,opt.hidden,opt.layers,opt.bidirec,drop_out=opt.dropout).to(device)
        if opt.restore == '':
        ...
        test_glue(model_bert, model, test_batch)
        ...
        for epoch in range(opt.epoch):
            ...
            train_loss, train_acc = train(model_bert, model, train_batch, optimizer, criterion)
            valid_loss, valid_acc = evaluate(model_bert, model, dev_batch, criterion)
