...
def generate(config, encoder, decoder, attention, dataloader):
    '''
    ...
    for batch in dataloader:
        ...
        prev_c  = torch.zeros((1, encoder.hidden_size), dtype=torch.float, requires_grad=False).to(config.device)
        prev_h  = torch.zeros((1, encoder.hidden_size), dtype=torch.float, requires_grad=False).to(config.device)
        outputs = torch.zeros((1, enc_len, encoder.hidden_size), dtype=torch.float, requires_grad=False).to(config.device)
        ...
        text_gen = [0]
        while True:
            prev_h, prev_c = decoder(prev_word,prev_h, prev_c)
            pred, attention_weights = attention(outputs, prev_h)
...
def train(config, encoder, decoder, attention, train_set, val_set):
    ...
    attention_optimizer = optim.RMSprop(attention.parameters(),  lr=lr_d, alpha=config.decay_rate)
    ...
    for epoch in range(config.epoch):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            outputs = torch.zeros((config.batch_size, enc_max_len, config.hidden_size), dtype=torch.float, requires_grad=False).to(config.device)
            ...
            for i in range(dec_max_len):
                h, c = decoder(symblos[:, i], h, c)
                pred, attention_weights = attention(outputs, h)
            ...
            torch.nn.utils.clip_grad_value_(attention.parameters(),config.grad_clip)
        ...
        res = generate(config, encoder, decoder, attention, val_dataloader)
        if best_valid_acc > res:
            best_valid_acc = res
            checkpoint = {}
            checkpoint["encoder"] = encoder
            checkpoint["decoder"] = decoder
            checkpoint["attention"] = attention
...
def test(config, encoder, decoder, attention, test_set):

    test_dataloader = DataLoader(test_set, batch_size=1, collate_fn=test_set.collate_fn)
    acc = generate(config, encoder,decoder,attention, test_dataloader)
...
def main():
    ...
    attention = model.Attention(config).to(config.device)
    # init parameters
    ...
    if config.mode != 'test':
        ...
        train(config, encoder, decoder, attention, train_set, val_set)
    else:
        if config.restore != '':
            checkpoint = torch.load(config.restore)
            encoder = checkpoint["encoder"]
            ...
            print('need --restore')
            exit()
        test(config, encoder, decoder, attention, test_set)
