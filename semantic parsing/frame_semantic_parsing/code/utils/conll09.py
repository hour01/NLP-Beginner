...
class CoNLL09Element:
    ...
    def __init__(self, conll_line):
        ...
        # lexical unit of target(target word lemma)
        self.lu = LUDICT.addstr(lufields[0])
        # pos of target(pos of the target word)
...
class CoNLL09Example(FrameSemParse):
    ...
    def __init__(self, sentence, elements):
        ...
        notfes = []
        self.invertedfes = {}
        # get the target in this example
        for e in elements:
            if e.is_pred:
                self.add_target((e.id - 1), e.lu, e.lupos, e.frame)

            # get the frame elements
            # invertedfes[role]=[begin of the role, end of the role]
