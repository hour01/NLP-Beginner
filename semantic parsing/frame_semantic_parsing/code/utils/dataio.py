...
def read_conll(conll_file):
    ...
    with codecs.open(conll_file, "r", "utf-8") as cf:
        ...
        for l in tqdm.tqdm(cf):
            l = l.strip()
            if l == "":
                # sentence object for different example is same
                if elements[0].sent_num != snum:
                    sentence = Sentence(elements=elements)
                    next_ex += 1
                ...
                elements = []
                continue
            elements.append(CoNLL09Element(l))
        cf.close()
    print("# examples in %s : %d in %d sents" % (conll_file, len(examples), next_ex))
    print("# examples with missing arguments : %d" % missingargs)
...
def create_target_frame_map(luIndex_file, tf_map):
    print("\nReading the frame - lexunit map from {}...\n".format(luIndex_file))
...
def read_related_lus():
    ...
    for f in tqdm.tqdm(os.listdir(FRAME_DIR)):
        ...
        # one lexical unit may evoke different frame.
        # what lu_to_frame_dict is to record the frames which can be evoked by this lexcal unit.
        # vice versa, so there it is frame_to_lu_dict.
        for l in lus:
            tot_lus += 1
    ...
    # the related_lus records those lexical unit which may evoke the same frame as the key lexical unit.
    related_lus = {}
